...
def index():
    ...
    if search_query:
        ...
        query = Entry.select().where(Entry.label1=='強化学習' or Entry.label2==label_query or Entry.label3==label_query)
    else:
...
#Create entries from github
def create_entries():
    for issue in issues:
        try:
            _, yaml_data, body = issue.body.split('---', 2)
            meta_data = yaml.load(yaml_data)
            labels=['']*3
            for index, label in enumerate(issue.labels):
                labels[index] = label.name if label.name!='論文紹介' else ''
    
            new_entry = Entry(
                # fields that use github data
                title=issue.title,
                publisher=issue.user.name,
                publisher_url=issue.user.html_url,
                issueurl=issue.url,
                ...
                # fields that use yaml meta data
                authors=meta_data['authors'],
                labs=meta_data['labs'],
                conference=meta_data['conference'],
                sport=meta_data['sport'],
                url=meta_data['url'],
...
def main():
    # Drop all tables <- Probably better to do something efficent in the future
    database.drop_tables([Entry])
    database.create_tables([Entry, FTSEntry], safe=True)
    create_entries()
    app.run(host='52.14.43.194', port=8080)
